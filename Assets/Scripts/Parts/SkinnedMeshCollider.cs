// https://qiita.com/UnagiHuman/items/7db6c75adea0d5862acfを引用、VRM表示時スケールがおかしくなることに対応するためUpdate内のSetMatrixを一部変更
...
public class SkinnedMeshCollider : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        _bakePointComputeShader.SetMatrix("LocalToWorld", _skinnedMeshRenderer.worldToLocalMatrix * _skinnedMeshRenderer.rootBone.localToWorldMatrix *
         // VRMを表示するとSkinnedmeshのMeshColliderのスケールがlocalScaleの2乗になってしまうため補正する
         Matrix4x4.Scale(new Vector3(1 / model.localScale.x, 1 / model.localScale.y, 1 / model.localScale.z)));
